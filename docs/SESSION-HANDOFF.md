...
`docs/Watcher_v2_Roadmap.pdf` (**v2.9**, regenerated this session from `docs/make_roadmap.py`) and
the five specs in `docs/specs/` — why the code is shaped the way it is. The one written this
session, `g3-emergency-path.md`, is the safety argument behind the whole emergency path and should
be read before anyone edits it.

**The roadmap generator is honest again, and that is worth one paragraph.** The v2.8 PDF
circulated while its generator stayed outside the repository — `docs/make_roadmap.py` was still at
v2.5, so nothing here could reproduce the document everyone was reading. This session re-derived
v2.8's content from the PDF itself, folded session 9 into it, and regenerated: generator and
artifact now agree at **v2.9**, 12 pages, same structure. **Never hand-edit the PDF** — edit the
generator and re-run `python docs/make_roadmap.py` (needs `reportlab`).
...
| Python files | **89 source**, 131 with tests; mypy checks 129 |
...
### The roadmap, regenerated

`docs/make_roadmap.py` was three revisions behind the PDF in circulation. It is now **v2.9**: v2.8's
twelve pages re-derived from the artifact, with G3 moved to DONE, Track G at 4.0d, the total at
**32.75d**, M1 at **~2.5 days**, session 9 in the log (cumulative 19.5d delivered), and the risk
register re-scored — the emergency risk closed and replaced by three narrower, named ones. Run
`python docs/make_roadmap.py` after any plan change; do not hand-edit the PDF.
